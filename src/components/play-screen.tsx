...
export function PlayScreen({
  ...
  // Prevents finish() from being processed more than once.
  // A ref is used because React state updates are asynchronous.
  ...
  const elapsed = Math.max(
    0,
    (tick - startedAt.current - pausedTotal.current) / 1000,
  );

  const timeLeft = session.timeLimit
  ...
  const finish = (won: boolean) => {
    // Hard guard against duplicate result/reward processing.
    ...
    const speed =
      won &&
      session.timeLimit &&
      elapsed < session.timeLimit * 0.5;

    const coins = won
      ? 50 +
        foundList.reduce(
          (n, w) => n + Math.max(0, w.length - 3) * 4,
          0,
        ) +
        (noHint ? 30 : 0) +
        (perfect ? 50 : 0) +
    ...
    const xp =
      (won ? 28 : 8) +
      foundList.length * 4 +
      (session.levelId?.includes("-8") ? 40 : 0);
  ...
  const onFound = (word: string) => {
    if (found.has(word) || endedRef.current) return;
    ...
    const nextCombo =
      now - lastFind.current < 6000 ? combo + 1 : 1;
  ...
  const remainingPlacements = puzzle.placements.filter(
    (p) => !found.has(p.word),
  );
            ...
            return (
              <li
                ...
                className={`rounded-full px-2.5 py-1 text-[11px] tracking-[0.14em] ${
                    ...
                    ? "bg-surface text-subtle line-through"
...
function HintBtn({
  ...
  return (
    ...
    >
      ...
      <span className="mt-0.5 text-[10px] text-fg">
        {label}
      </span>
